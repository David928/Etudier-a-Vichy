...
class EcoleAnnotationView: MKAnnotationView {
    ...
    func setupCenter() -> UIView? {
        guard let anno = annotation as? EcoleAnnotation else { return nil}
        ...
        imageView.image = anno.ecole.image
    ...
    @objc func detail() {
        guard let anno = annotation as? EcoleAnnotation else { return }
        
        //controller?.toDetail(loisir: anno.ecole)
        
        NotificationCenter.default.post(name: Notification.Name("detail"), object: anno.ecole)
    }
